...
def linear_model_main(X_parameters,Y_parameters,predict_input):
 ...
 regr = linear_model.LassoCV(fit_intercept=True, tol=0.01, normalize=False, cv=10)
...
# print ave_mae
print ("Lasso MAE: %f" % (np.mean(mean_mae)))
print ("Lasso MAE STD: %f" % (np.mean(std_mae)))
print ("Lasso Pear: %f" % (np.mean(mean_pear)))
print ("Lasso Pear STD: %f" % (np.mean(std_pear)))
