...
def linear_model_main(X_parameters,Y_parameters,predict_input):
 ...
 regr = linear_model.ElasticNetCV(fit_intercept=True, normalize=False, l1_ratio=0.5, tol=0.01, cv=10, max_iter=1000)
...
print ("ElasticNet MAE: %f" % (np.mean(mean_mae)))
print ("ElasticNet MAE STD: %f" % (np.mean(std_mae)))
print ("ElasticNet Pear: %f" % (np.mean(mean_pear)))
print ("ElasticNet Pear STD: %f" % (np.mean(std_pear)))
